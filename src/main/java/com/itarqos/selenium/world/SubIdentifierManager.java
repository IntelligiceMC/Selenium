package com.itarqos.selenium.world;

import com.itarqos.selenium.mixin.render.WorldRendererAccessor;
...
public final class SubIdentifierManager {
    ...
    private boolean debounce(ChunkId id, int sliceIndex) {
        ...
        int debounceMs = SeleniumClient.CONFIG != null ? Math.max(0, SeleniumClient.CONFIG.sliceDebounceMillis) : 200;
    ...
    public void flushVisible() {
        ...
        if (SeleniumClient.CONFIG != null && !SeleniumClient.CONFIG.enablePartialMeshing) {
                ...
                for (int slice = bits.nextSetBit(0); slice >= 0; slice = bits.nextSetBit(slice + 1)) {
                    if (arr != null && arr[slice] != null) {
                        for (long lp : arr[slice]) {
                            ...
                            ((WorldRendererAccessor) wr).selenium$invokeScheduleSectionRender(origin, false);
        ...
        int baseSliceBudget = SeleniumClient.CONFIG != null ? Math.max(0, SeleniumClient.CONFIG.sliceBudgetPerTick) : 24;
        int sliceBudget = (int)Math.floor(baseSliceBudget * FrameBudgetController.get().getUnhideMultiplier());
        if (FrameBudgetController.get().wasLongFrame() && SeleniumClient.CONFIG != null && SeleniumClient.CONFIG.enableMicroStutterGuard) {
            sliceBudget = 0; // freeze new work after a long frame
        }

        // Turn-bias prefetch widening
        double turnBiasStrength = (SeleniumClient.CONFIG != null && SeleniumClient.CONFIG.enableTurnBiasPrefetch)
                ? Math.max(0.0, Math.min(1.0, SeleniumClient.CONFIG.turnBiasStrength))
                : 0.0;
        ...
        while (it.hasNext()) {
            ...
            for (int slice = bits.nextSetBit(0); slice >= 0; slice = bits.nextSetBit(slice + 1)) {
                ...
                boolean allowPrefetch = SeleniumClient.CONFIG == null || SeleniumClient.CONFIG.enablePredictivePrefetch;
                ...
                if (!(culledBehind || tooFar) || (allowPrefetch && inForwardCone && dist < farCutoff * 1.15)) {
                    ...
                    boolean useGradual = SeleniumClient.CONFIG != null && SeleniumClient.CONFIG.enableVisibilityDeprioritization;
                    int threshold = SeleniumClient.CONFIG != null ? Math.max(0, SeleniumClient.CONFIG.hiddenDeprioritizeFrames) : 30;
        ...
        if (!candidates.isEmpty() && sliceBudget > 0) {
            boolean useScreenSpace = SeleniumClient.CONFIG == null || SeleniumClient.CONFIG.enableScreenSpaceBudgeter;
            if (useScreenSpace) {
        ...
        if (SeleniumClient.CONFIG == null || !SeleniumClient.CONFIG.enableVisibilityDeprioritization) {
            pendingUnhide.clear();
        } else {
            int budget = Math.max(0, SeleniumClient.CONFIG.unhidePerTick);
            // Apply QoS scaling and micro-stutter guard
            budget = (int)Math.floor(budget * FrameBudgetController.get().getUnhideMultiplier());
            if (FrameBudgetController.get().wasLongFrame() && SeleniumClient.CONFIG.enableMicroStutterGuard) {
                budget = 0;
            }
            int processed = 0;
            while (processed < budget && !pendingUnhide.isEmpty()) {
                BlockPos pos = pendingUnhide.pollFirst();
                ((WorldRendererAccessor) wr).selenium$invokeScheduleSectionRender(pos, false);
                CullingStats.incSliceFlushed();
                processed++;
            }
        }
