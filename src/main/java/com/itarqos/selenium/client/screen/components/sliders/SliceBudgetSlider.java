...
public class SliceBudgetSlider extends SliderWidget {
    private static final int MIN = 1;
    private static final int MAX = 64;
    private final SeleniumConfig cfg;

    public SliceBudgetSlider(int x, int y, int width, int height, int initial, SeleniumConfig cfg) {
    ...
    protected void applyValue() {
        cfg.sliceBudgetPerTick = current();
    ...
    private static String label(int v) {
        return Text.translatable("selenium.settings.slice_budget_per_tick", clamp(v)).getString();
    }
}
