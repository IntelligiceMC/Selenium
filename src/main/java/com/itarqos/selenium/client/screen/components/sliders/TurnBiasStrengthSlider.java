...
public class TurnBiasStrengthSlider extends SliderWidget {
    private static final double MIN = 0.0;
    private static final double MAX = 1.0;
    private final SeleniumConfig cfg;

    public TurnBiasStrengthSlider(int x, int y, int width, int height, float initial, SeleniumConfig cfg) {
    ...
    private static String label(float v) {
        return Text.translatable("selenium.settings.turn_bias_strength", String.format("%.2f", clamp(v))).getString();
    }
}
