package com.itarqos.selenium.render;
...
public class RenderTaskScheduler {
    ...
    public void submitTask(RenderTask task) {
        ...
        SeleniumLog.info("[RenderScheduler] Task submitted: %s (priority: %s)", 
    ...
    private void processQueue(Queue<RenderTask> queue, String priorityName) {
        while (!queue.isEmpty() && hasTimeRemaining()) {
            ...
            if (!task.isRelevant()) {
                ...
                SeleniumLog.info("[RenderScheduler] Skipped irrelevant task: %s", task.name);
                continue;
            }
            ...
            try {
                ...
                SeleniumLog.info("[RenderScheduler] Executed %s task '%s' in %.2fms", 
                    priorityName, task.name, taskDuration / 1_000_000.0);
            } catch (Exception e) {
                SeleniumLog.error("[RenderScheduler] Task '%s' failed: %s", 
    ...
    private void logStatistics() {
        ...
        SeleniumLog.info("[RenderScheduler] Stats - Queued: %d (H:%d N:%d L:%d), Processed: %d, Skipped: %d",
    ...
    public void clear() {
        ...
        SeleniumLog.info("[RenderScheduler] Cleared %d pending tasks", cleared);
    }
