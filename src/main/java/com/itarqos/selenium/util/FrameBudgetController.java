package com.itarqos.selenium.util;
...
public final class FrameBudgetController {
    ...
    private void updateLongFrameFlag() {
        if (SeleniumClient.CONFIG == null) { longFrame = false; return; }
        if (!SeleniumClient.CONFIG.enableMicroStutterGuard) { longFrame = false; return; }
        int threshold = Math.max(10, SeleniumClient.CONFIG.microStutterThresholdMs);
    ...
    private double aggr() {
        if (SeleniumClient.CONFIG == null) return 0.0;
        double a = Math.max(0.0, Math.min(1.0, SeleniumClient.CONFIG.qosAggressiveness));
        return a;
    }

    private double targetMs() {
        if (SeleniumClient.CONFIG == null) return 8.0; // ~125 FPS default
        int fps = Math.max(30, Math.min(240, SeleniumClient.CONFIG.targetFps));
        return 1000.0 / fps;
